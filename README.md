...
`-C` because OpenSSH doesn't compress by default, and the file is 1120 KB of
repetitive text: it goes over the wire at 340 KB with compression on, for the
cost of one flag.
...
Stripped and compressed together it's 231 KB instead of 1120. That is 79%
...
- **2002 tests**, and a test that fails if any of them asserts nothing at all.
...
- `test_faultone.py`: `python3 test_faultone.py`, 2002 tests, no dependencies
