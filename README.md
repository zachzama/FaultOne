...
`-C` because OpenSSH doesn't compress by default, and the file is 1103 KB of
repetitive text: it goes over the wire at 334 KB with compression on, for the
cost of one flag.
...
Stripped and compressed together it's 228 KB instead of 1103. That is 79%
...
- **1948 tests**, and a test that fails if any of them asserts nothing at all.
- **345 mutations**, each one breaking a rule on purpose. Every one has to make
...
- `test_faultone.py`: `python3 test_faultone.py`, 1948 tests, no dependencies
