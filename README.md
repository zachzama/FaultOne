...
`-C` because OpenSSH doesn't compress by default, and the file is 1092 KB of
repetitive text: it goes over the wire at 330 KB with compression on, for the
cost of one flag.
...
ssh -C -J jump user@box "python3 - --report" < /tmp/faultone.py    # 226 KB on the wire
```

That needs Python 3.9 on **your** machine, and so does the box.
Stripped and compressed together it's 226 KB instead of 1092. That is 79%
...
- **1901 tests**, and a test that fails if any of them asserts nothing at all.
- **312 mutations**, each one breaking a rule on purpose. Every one has to make
...
- `test_faultone.py`: `python3 test_faultone.py`, 1901 tests, no dependencies
