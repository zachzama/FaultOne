...
`-C` because OpenSSH doesn't compress by default, and the file is 756 KB of
repetitive text: it goes over the wire at 228 KB with compression on, for the
cost of one flag.
...
ssh -C -J jump user@box "python3 - --report" < /tmp/faultone.py    # 163 KB on the wire
```

That needs Python 3.9 on **your** machine; the box still only needs 3.7.
Stripped and compressed together it's 163 KB instead of 756. That is 78%
...
- `test_faultone.py`: `python3 test_faultone.py`, 1213 tests, no dependencies
