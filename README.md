...
`-C` because OpenSSH doesn't compress by default, and the file is 1091 KB of
...
Stripped and compressed together it's 226 KB instead of 1091. That is 79%
...
- **1896 tests**, and a test that fails if any of them asserts nothing at all.
- **306 mutations**, each one breaking a rule on purpose. Every one has to make
...
- `test_faultone.py`: `python3 test_faultone.py`, 1896 tests, no dependencies
