...
`-C` because OpenSSH doesn't compress by default, and the file is 1094 KB of
repetitive text: it goes over the wire at 331 KB with compression on, for the
cost of one flag.
...
Stripped and compressed together it's 226 KB instead of 1094. That is 79%
...
- **1909 tests**, and a test that fails if any of them asserts nothing at all.
- **320 mutations**, each one breaking a rule on purpose. Every one has to make
...
- `test_faultone.py`: `python3 test_faultone.py`, 1909 tests, no dependencies
