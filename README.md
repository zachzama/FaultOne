...
`-C` because OpenSSH doesn't compress by default, and the file is 1065 KB of
repetitive text: it goes over the wire at 322 KB with compression on, for the
cost of one flag.
...
ssh -C -J jump user@box "python3 - --report" < /tmp/faultone.py    # 220 KB on the wire
```

That needs Python 3.9 on **your** machine, and so does the box.
Stripped and compressed together it's 220 KB instead of 1065. That is 79%
...
- `test_faultone.py`: `python3 test_faultone.py`, 1848 tests, no dependencies
