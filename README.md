...
**198 findings it can reach. One line saying which one to fix first.**
...
`-C` because OpenSSH doesn't compress by default, and the file is 1071 KB of
repetitive text: it goes over the wire at 324 KB with compression on, for the
cost of one flag.
...
Stripped and compressed together it's 222 KB instead of 1071. That is 79%
...
**41 things are inspected**, and **198 distinct conclusions** can come out of
them: 164 are faults, 34 are context.
...
- `test_faultone.py`: `python3 test_faultone.py`, 1855 tests, no dependencies
