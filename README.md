...
`-C` because OpenSSH doesn't compress by default, and the file is 986 KB of
repetitive text: it goes over the wire at 297 KB with compression on, for the
cost of one flag.
...
Stripped and compressed together it's 208 KB instead of 986. That is 79%
...
- `test_faultone.py`: `python3 test_faultone.py`, 1689 tests, no dependencies
