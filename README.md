...
**188 findings it can reach. One line saying which one to fix first.**
...
`-C` because OpenSSH doesn't compress by default, and the file is 980 KB of
repetitive text: it goes over the wire at 295 KB with compression on, for the
cost of one flag.
...
ssh -C -J jump user@box "python3 - --report" < /tmp/faultone.py    # 206 KB on the wire
```

That needs Python 3.9 on **your** machine; the box still only needs 3.7.
Stripped and compressed together it's 206 KB instead of 980. That is 79%
...
**41 things are inspected**, and **188 distinct conclusions** can come out of
them: 155 are faults, 33 are context.
...
- `test_faultone.py`: `python3 test_faultone.py`, 1679 tests, no dependencies
