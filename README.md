...
`-C` because OpenSSH doesn't compress by default, and the file is 1098 KB of
...
Stripped and compressed together it's 227 KB instead of 1098. That is 79%
...
- **1928 tests**, and a test that fails if any of them asserts nothing at all.
- **333 mutations**, each one breaking a rule on purpose. Every one has to make
...
- `test_faultone.py`: `python3 test_faultone.py`, 1928 tests, no dependencies
