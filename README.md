...
`-C` because OpenSSH doesn't compress by default, and the file is 1097 KB of
repetitive text: it goes over the wire at 332 KB with compression on, for the
cost of one flag.
...
ssh -C -J jump user@box "python3 - --report" < /tmp/faultone.py    # 227 KB on the wire
```

That needs Python 3.9 on **your** machine, and so does the box.
Stripped and compressed together it's 227 KB instead of 1097. That is 79%
...
- **1922 tests**, and a test that fails if any of them asserts nothing at all.
- **328 mutations**, each one breaking a rule on purpose. Every one has to make
...
- `test_faultone.py`: `python3 test_faultone.py`, 1922 tests, no dependencies
