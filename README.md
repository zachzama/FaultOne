...
`-C` because OpenSSH doesn't compress by default, and the file is 1101 KB of
repetitive text: it goes over the wire at 333 KB with compression on, for the
cost of one flag.
...
Stripped and compressed together it's 227 KB instead of 1101. That is 79%
...
- **1943 tests**, and a test that fails if any of them asserts nothing at all.
- **340 mutations**, each one breaking a rule on purpose. Every one has to make
...
- `test_faultone.py`: `python3 test_faultone.py`, 1943 tests, no dependencies
