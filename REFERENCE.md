...
difference is which one it puts at the top, and that is the whole product: 185
...
| **Findings** | **185** | Distinct conclusions it can reach and state in plain language. 152 are faults; 33 are context, like which switch port you're on. |
| **Ranked causes** | **152** | Findings the verdict knows how to rank and assign an owner to. |
| **Automated tests** | **531** | 1621 tests of this program's own code. A developer number, not a measure of what it checks for you. |

**The 185 findings are the useful figure** if you want to know what the tool can
...
python3 test_faultone.py           # 1621 tests, a few seconds, no dependencies
```
...
1621 tests, no dependencies, no network, a few seconds, so they run
...
> way out - and who owns it. Ranks 185 findings with readable rules instead of
