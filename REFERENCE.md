...
difference is which one it puts at the top, and that is the whole product: 201
...
### And which hop the queue is on

`mtr` reports a Best per hop alongside the average, and nothing had ever read
it. The same split works one hop at a time: the best time is what the hop costs
with nothing queued, and the average less that is time packets spent waiting.
Cumulative, like the timings themselves, so the waiting a hop *adds* is its
increase over the hop before it.

`queue_builds_at_hop` names the hop holding most of that, with the AS where the
...
or below. "Different check" is decided by the finding's family, which is its
name with a hand-kept list of exceptions - so it catches two readings that are
...
| **Findings** | **201** | Distinct conclusions it can reach and state in plain language. 167 are faults; 34 are context, like which switch port you're on. |
| **Ranked causes** | **167** | Findings the verdict knows how to rank and assign an owner to. |
| **Automated tests** | **531** | 1901 tests of this program's own code. A developer number, not a measure of what it checks for you. |

**The 201 findings are the useful figure** if you want to know what the tool can
...
26. Target reachability and loss
27. Hop-by-hop path (traceroute, or mtr where installed)
28. TCP-probe path, when the standard one is filtered
29. Path MTU
30. DNS resolution
31. Each configured DNS resolver, individually
...
39. This box's own service, asked over HTTP for an answer rather than a connection
40. Proxy configuration, how this box is told to reach the internet: the
...
python3 test_faultone.py           # 1901 tests, a few seconds, no dependencies
```
...
```
FaultOne 1.25.0 - Linux - python 3.11.2 - 2026-08-06T15:16:44-07:00
```
...
```
FaultOne 1.25.0 - Linux - 2026-08-06T14:58:06-07:00
```
...
(interface MTU, then 1492/1474/1280/1000: the common tunnel sizes) and
...
PATH MTU TO 8.8.8.8
   ...
   1492 bytes   blocked
   1474 bytes   passes
  -> largest that gets through: 1474  (interface is set to 1500)
```
...
### Which side of the proxy went quiet, and whether that is the network

Loss is a percentage, so it needs traffic to measure. A side that has stopped
...
say sends none while its kernel goes on acknowledging everything that arrives.
Read on `lastsnd` and `lastrcv` alone, these two are the same reading:
...
A kernel that does not report `lastack` gets neither verdict rather than a
guess. Without it the two rows above are genuinely indistinguishable, and
naming one of them would be a coin toss drawn as a measurement.
...
- **`systemctl`**: what the box is configured to run and is not running. It is
  ...
  The failed units are then split by whether this box says anything about them
  beyond their name. A failed `.socket` unit is a port systemd was holding and
  is not, and a failed `.service` whose own program holds listening sockets
  here is one dead member of a working set - both are graded, both name their
  ground, and neither asks the reader to judge. Everything else keeps the
  warning that says a failed timer is housekeeping, a failed instance is an
  outage, and nothing available here tells them apart. The match between a unit
  and a process is a prefix in either direction, because `ss` reports a process
  by its comm and the kernel truncates that at 15 characters.
...
1901 tests, no dependencies, no network, a few seconds, so they run
...
> way out - and who owns it. Ranks 201 findings with readable rules instead of
