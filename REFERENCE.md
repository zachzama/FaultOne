...
| **Automated tests** | **531** | 1238 tests of this program's own code. A developer number, not a measure of what it checks for you. |
...
python3 test_faultone.py           # 1238 tests, a few seconds, no dependencies
```
...
- `dmesg` first, with its default `[  1238.567]` seconds-since-boot stamps.
...
1238 tests, no dependencies, no network, a few seconds, so they run
