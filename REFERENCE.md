...
| **Findings** | **187** | Distinct conclusions it can reach and state in plain language. 153 are faults; 34 are context, like which switch port you're on. |
| **Ranked causes** | **153** | Findings the verdict knows how to rank and assign an owner to. |
| **Automated tests** | **531** | 1655 tests of this program's own code. A developer number, not a measure of what it checks for you. |
...
python3 test_faultone.py           # 1655 tests, a few seconds, no dependencies
```
...
1655 tests, no dependencies, no network, a few seconds, so they run
