...
| **Automated tests** | **531** | 1250 tests of this program's own code. A developer number, not a measure of what it checks for you. |
...
python3 test_faultone.py           # 1250 tests, a few seconds, no dependencies
```
...
- `dmesg` first, with its default `[  1250.567]` seconds-since-boot stamps.
...
1250 tests, no dependencies, no network, a few seconds, so they run
