...
function hopList(col, names){
  ...
  return `<div class="hops" title="${escapeHtml(hopWhy(col))}">${col.hops.map(h => `
      ...
      <div class="hrow ${cls(h.state === 'ok' ? '' : h.state)}">
        ...
        // timing - where an AS appended to the address made its width vary and
        // pushed the bar about. Drawn when there is either, not only when
        // reverse DNS answered: hung off the name it would have disappeared on
        // every hop without a PTR, which is most of them on a path that leaves
        // the site.
        (h.asn || names[h.host]) ? `<div class="hname">${
          h.asn ? `<span class="hasn">${escapeHtml(h.asn)}</span>` : ''}${
          h.asn && names[h.host] ? ' \u00b7 ' : ''}${
          names[h.host] ? escapeHtml(names[h.host]) : ''}</div>` : ''}${
