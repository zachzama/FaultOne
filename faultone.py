...
// The way in, coloured by what the report concluded about it rather than by
// re-reading one counter. The node keyed on client loss alone, so a direction
// marked degraded for jitter or for queuing delay - neither of which is loss -
// drew green here while the zone beside it read DEGRADED. The zone is the
// conclusion about this direction and this is the picture of it; thresholds in
// the picture are a second copy of the ones in the analysis, and two copies of
// a threshold are two chances to disagree. The measured loss and rtt stay in
// the node's own line, where they are readings rather than a judgement.
const ZONE_SEV = {pass: 'ok', warn: 'warn', fail: 'crit', skip: 'ok'};
function clientSeverity(data){
  const z = (data.sides || []).find(s => s.side === 'downstream');
...
function renderHopChain(data){
  ...
  const inboundHtml = cin ? `
    <div class="inbound">
      ...
      <div class="hop-chain">
        <div class="hop-node ${clientSeverity(data)}">
